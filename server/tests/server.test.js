...
describe('POST /todos', ()=>{
  ...
  it('should not create todo with invalid body data', (done)=>{
    ...
    request(app).post('/todos').send({eText})
      .expect(400)
      .end((err, res)=>{
        if (err) {
          return done(err)
...
describe('PATCH /todos/:id', ()=>{
  it('should update the todo', (done)=>{
    ...
    request(app).patch(`/todos/${hexID}`).send({text: text, completed: true})
      .expect(200)
      .expect((res)=>{
  ...
  it('should clear completedAt when todo is not completed', (done)=>{
    ...
    request(app).patch(`/todos/${hexID}`).send({text: text, completed: false})
      .expect(200)
      .expect((res)=>{
        expect(res.body.todo.text).toBe(text);
